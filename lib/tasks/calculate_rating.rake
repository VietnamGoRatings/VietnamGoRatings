namespace :rating do
  ...
  task calculate: :environment do
    ...
    whr = WholeHistoryRating::Base.new(w2: 4)
    ...
    women_whr = WholeHistoryRating::Base.new(w2: 4)
    ...
    ActiveRecord::Base.transaction do
      ...
      Player.where(is_male: 1).all.each do |player|
        rating_tuple = (whr.ratings_for_player(player.id).first)
      ...
      Player.where(is_male: 0).all.each do |player|
        rating_tuple = (women_whr.ratings_for_player(player.id).first)
        if (rating_tuple != nil) and (rating_tuple.size > 0)
          rating = rating_tuple[1]
          player.rating = rating + 1500
          player.save!
